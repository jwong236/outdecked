...
export interface DeckBuilderSearchCardProps {
  card: ExpandedCard;
  ...
  showRarity?: boolean;
  priority?: boolean;
  isInDeck?: boolean; // Explicit prop to indicate if card is in deck
}

export function DeckBuilderSearchCard({ 
  card, 
  onClick, 
  onAddToDeck,
  onQuantityChange,
  ...
  return (
    ...
    >
      ...
      <div className="relative aspect-[3/4] mb-3 rounded-lg overflow-hidden bg-gray-800">
        ...
        {/* Quantity Overlay - only show if card has quantity > 0 */}
        {(card.quantity || 0) > 0 && (
