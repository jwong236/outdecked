...
export function DefaultFilters({ className = '' }: DefaultFiltersProps) {
  ...
  const toggleBasicPrintsOnlyFilter = (enabled: boolean) => {
    if (enabled) {
      ...
      addFilter({ type: 'or', field: 'print_type', value: 'Base', displayText: 'Base Prints Only' });
      addFilter({ type: 'or', field: 'print_type', value: 'Starter Deck', displayText: 'Base Prints Only' });
    } else {
      // Remove both Base and Starter Deck filters
      const filtersToRemove = searchPreferences.filters
        .map((filter, index) => ({ filter, index }))
        .filter(({ filter }) => 
          filter.field === 'print_type' && 
